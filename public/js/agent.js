...
export function setLockMode(enabled) {
    ...
    if (badge) badge.textContent = LOCK_MODE ? "Lock: ON (rotation Y seulement)" : "Lock: OFF";
}
...
function applyLockToAgent(agent, enabled, snap = true) {
    ...
    if (!b) return;
    if (enabled) {
        if (!agent.anchor) agent.anchor = new THREE.Vector3(agent.mesh.position.x, 0.5, agent.mesh.position.z);
        ...
        if (snap) {
            ...
            tr.setOrigin(new AmmoLib.btVector3(agent.anchor.x, agent.anchor.y ?? 0.5, agent.anchor.z));
            if (typeof b.setWorldTransform === "function") b.setWorldTransform(tr);
            else if (typeof b.setCenterOfMassTransform === "function") b.setCenterOfMassTransform(tr);
        }
...
export function faceMotion(a, dt) {
    ...
    let diff = THREE.MathUtils.euclideanModulo(targetYaw - yaw + Math.PI, Math.PI * 2) - Math.PI;
    const desiredRate = THREE.MathUtils.clamp(diff / Math.max(dt, 1e-3), -maxYawRate, maxYawRate);
...
export function spawnAgent(roleKey, factionIndex, x, y, z) {
    ...
    const body = createRigidBody(mesh, shape, role.mass, mesh.position, mesh.quaternion);

    if (typeof body.setAngularFactor === "function") body.setAngularFactor(TMP.angY);
    if (typeof body.setActivationState === "function") body.setActivationState(4);
...
export function updateAgent(a, dt, resources, removeResource) {
    ...
    if (a.state === "wander") {
        ...
        if (res) {
            ...
            if (d < 0.7) { // RES_PICK
                a.hp = Math.min(66, a.hp + 20); // RES_HEAL
                removeResource(res);
            }
        } else {
            steerWander(a, dt);
...
function steerWander(a, dt) {
    ...
    if (a.turnT <= 0) {
        ...
        a.headingDeg = (a.headingDeg + (Math.random() * 2 - 1) * a.turnRate * 0.3 + 360) % 360;
    }
    const rad = (a.headingDeg * Math.PI) / 180;
    const ax = Math.sin(rad), az = Math.cos(rad);
    const vx = ax * a.speed, vz = az * a.speed;
...
function keepAnchored(a) {
    const AmmoLib = getAmmo();
    if (!a.anchor) a.anchor = new THREE.Vector3(a.mesh.position.x, 0.5, a.mesh.position.z);

    const b = a.body;
    const tr = TMP.trans;
    b.getMotionState().getWorldTransform(tr);
    tr.setOrigin(new AmmoLib.btVector3(a.anchor.x, a.anchor.y ?? 0.5, a.anchor.z));
