...
export function createHouses() {
    ...
    for (let i = 0; i < FACTIONS.length; i++) {
        ...
        const houseGeo = new THREE.BoxGeometry(HOUSE_SIZE, HOUSE_SIZE, HOUSE_SIZE);
        ...
        const shape = new AmmoLib.btBoxShape(new AmmoLib.btVector3(HOUSE_SIZE * 0.5, HOUSE_SIZE * 0.5, HOUSE_SIZE * 0.5));
        createRigidBody(houseMesh, shape, 0, houseMesh.position, houseMesh.quaternion);
        ...
        const frontVector = new THREE.Vector3(0, 0, -1).applyQuaternion(houseMesh.quaternion);
        const sideVector = new THREE.Vector3().crossVectors(new THREE.Vector3(0, 1, 0), frontVector).normalize();

        const spawnCenter = houseMesh.position.clone().add(frontVector.multiplyScalar(spawnDistance));
        ...
        for (let j = 0; j < agentRoles.length; j++) {
            ...
            const spawnPoint = spawnCenter.clone().add(sideVector.clone().multiplyScalar(offset));
...
export function getSpawnPointForAgent(factionId, role) {
    ...
    return new THREE.Vector3((Math.random() - 0.5) * 10, 5, (Math.random() - 0.5) * 10);
}